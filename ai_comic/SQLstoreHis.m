...
+(sqlite3*)openDB
{
    ...
    NSString *filePath = [docPath stringByAppendingPathComponent:@"hisDATA.sqlite"];
    int state = sqlite3_open([filePath UTF8String], &db);
    if (state != SQLITE_OK) {
        NSLog(@"数据库打开失败");
        return nil;
    }
//    NSLog(@"hisDataFilePath = %@",filePath);
